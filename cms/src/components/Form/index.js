        ...
        <div className="bg-white rounded-lg shadow-lg h-full grid p-2">
            ...
            {item.image && (
              <div className="w-full flex items-center justify-between mb-2 border border-gray-200 p-2 rounded-md">
                <img
                  ...
                  alt="Uploaded"
                  className="w-16 h-16 object-cover rounded-md mr-2 shadow-sm"
                />
              ...
              if (typeof value === 'string') {
                ...
                const fieldLabel =
                  dataSets
                    ...
                    .find((field) => field.id === key)?.label || key;

                return (
                  <div
                    key={key}
                    className="text-sm text-gray-600 flex items-center justify-between border border-gray-200 p-2 rounded-md"
                  >
                    ...
                    <div className="flex space-x-2">
                      <IconButton
                        ...
                        iconName="Edit"
                        onClick={() =>
                          handleIndividualEdit(item[identifierField], key, value)
                        }
                        label={`Edit ${fieldLabel}`}
                        className="text-blue-800 hover:text-gray-800 transition-all duration-200"
                      />
                      <IconButton
                        IconComponent={TrashIcon}
                        iconName="Delete"
                        onClick={() =>
                          handleDeleteField(item[identifierField], key)
                        }
                        label={`Delete ${fieldLabel}`}
                        className="text-red-600 hover:text-red-800 transition-all duration-200"
                      />
                    </div>
                  </div>
          ...
          <div className="flex space-x-4 mt-2 p-2">
            <IconButton
              ...
              label={`Edit Entry ${item[identifierField]}`}
              className="text-blue-800 hover:text-gray-800 transition-all duration-200"
            />
            ...
            <IconButton
              IconComponent={TrashIcon}
              iconName="Delete all"
              onClick={() => handleDelete(item[identifierField])}
              label={`Delete ${item[identifierField]}`}
              className="text-red-600 hover:text-red-800 transition-all duration-200"
            />
          </div>
        </div>
...
const Form = ({
  ...
  const handleIndividualSubmit = async (event) => {
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item[identifierField] === individualEdit.itemIdentifier
            ? response.data
            : item
        )
      );
  ...
  const handleSubmit = async (event) => {
    ...
    const hasErrors = dataSets.some(({ fields }) =>
    ...
    if (newEntries.length > 0) {
      ...
      try {
        ...
        setData((prevData) =>
          prevData.map((item) =>
            item[identifierField] === currentEditId ? response.data : item
          )
        );
  ...
  const handleDeleteField = async (identifier, fieldId) => {
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item[identifierField] === identifier ? response.data : item
        )
      );
            ...
            {individualEdit && (
              <div className="space-y-4">
                ...
                <FieldRenderer
                  ...
                  setFields={(fields) =>
                    fields.map((f) =>
                      f.id === individualEdit.field.id
                        ? { ...f, value: individualEdit.field.value }
                        : f
                    )
                  }
