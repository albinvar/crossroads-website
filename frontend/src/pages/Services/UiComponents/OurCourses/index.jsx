...
const OurCourses = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setLoading(true);
                const [ourCoursesResponse, titleResponse] = await Promise.all([
                    apiService.getServiceOurCourses(),
                    apiService.getServiceOurCoursesTitle(),
                ]);

                if (isMounted) {
                    const transformedValues = ourCoursesResponse.data
                        .map((course) => ({
                            image: course.image,
                            title: course.title,
                    ...
                    <>
                        <Slider ref={sliderRef} {...sliderSettings}>
                            {values.map((value, index) => (
                                <Link to={course.link} key={index} className='scale-[1] hover:scale-[1.05] transition-all duration-300 p-4'>
                                    <motion.div
                                        initial="hidden"
                                        animate="visible"
                                        variants={textVariants}
