...
const OurValues = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setLoading(true);
                const [ourValuesResponse, titleResponse] = await Promise.all([
                    apiService.getAboutOurValues(),
                    apiService.getAboutOurValuesTitle(),
                ]);

                if (isMounted) {
                    const transformedValues = ourValuesResponse.data
                        .map((value) => ({
                            image: value.image,
                            title: value.title,
    ...
    if (values === null || title === null || description === null) {
        return (
            <motion.div
                    ...
                    <>
                        <Slider ref={sliderRef} {...sliderSettings}>
                            {values.map((value, index) => (
                                <Link to={value.link} key={index} className='scale-[1] hover:scale-[1.05] transition-all duration-300 p-4'>
                                    <motion.div
                                        initial="hidden"
                                        animate="visible"
                                        variants={textVariants}
                                    ...
                                    >
                                        <div className="h-full text-left value-card rounded-lg overflow-hidden">
                                            <div className="relative">
                                                <img
                                                    src={value.image}
                                                    alt={`${value.title.replace(/<[^>]+>/g, '')} Image`}
                                                    className="rounded-lg transition-all duration-300 w-full h-48 object-cover"
                                                    onError={(e) => {
                                                        e.target.src = 'https://via.placeholder.com/300x200?text=Value+Image+Not+Found';
                                                        console.error(`Failed to load image: ${value.image}`);
                                                    }}
                                                />
                                            </div>
